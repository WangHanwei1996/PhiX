...
// ===========================================================================
// CUDA kernels: [0,1] clamp on d_curr
// ===========================================================================
...
// ===========================================================================
// Glass fraction  (fraction of physical cells with η > threshold)
// ===========================================================================
static double computeGlassFraction(const PhiX::ScalarField& eta,
                                    int nx, int ny,
                                    double threshold = 0.5)
{
    ...
    for (int j = 0; j < ny; ++j)
        for (int i = 0; i < nx; ++i) {
            const int idx = (i + g) + sx * ((j + g) + sy * g);
            if (eta.curr[idx] > threshold) ++count;
        }
    return static_cast<double>(count) / (nx * ny);
}
...
int main(int argc, char* argv[])
{
    ...
    // -----------------------------------------------------------------------
    // Noise generator (one curand state per physical cell)
    // -----------------------------------------------------------------------
    NoiseGenerator noiseGen(eta, noise_seed);
    ...
    if (start_step == 0)
        fout << "# step  time[s]  free_energy[J]  glass_fraction\n";

    // Write F at step 0 (cold start)
    if (start_step == 0) {
        const double F0 = computeFreeEnergy(eta, nx, ny, dx, dy,
                                             dFSR, w_eta, beta);
        const double gf0 = computeGlassFraction(eta, nx, ny);
        fout << 0 << "  " << 0.0 << "  " << F0 << "  " << gf0 << "\n";
        fout.flush();
    }
    ...
    const double act_threshold = 0.116;
    const int    total_cells   = nx * ny;
    const double P_activate    = std::erfc(act_threshold /
                                           (std::sqrt(2.0) * noise_std)) / 2.0;
    double expected_cumul = total_cells *
                            (1.0 - std::pow(1.0 - P_activate, (double)start_step));
    std::cout << "  P_activate/step = " << P_activate
              << "  E_cumul(" << start_step << ") = " << expected_cumul << "\n";
    std::ofstream fcount("output/activation_count.dat",
                         start_step == 0 ? std::ios::trunc : std::ios::app);
    if (!fcount)
        std::cerr << "Warning: cannot open output/activation_count.dat\n";
    if (start_step == 0)
        fcount << "# step  time[s]  expected_cumulative_activations\n";
    fcount << std::scientific << std::setprecision(6);

            
    noiseGen.apply(eta, NoiseType::GAUSSIAN, noise_mean, noise_std);
    ...
    for (int s = start_step; s < nSteps; ++s) {
        ...
        if (writer.shouldWrite(eqEta.step)) {
            ...
            // 计算并写出自由能和玻璃相比例（writeFields 已将 d_curr 下载到 curr）
            const double F  = computeFreeEnergy(eta, nx, ny, dx, dy,
                                                dFSR, w_eta, beta);
            const double gf = computeGlassFraction(eta, nx, ny);
            fout << eqEta.step << "  " << eqEta.time << "  " << F << "  " << gf << "\n";
            fout.flush();
        }



        // 按随机期望更新累计激活点数： E_n = E_{n-1} + (N - E_{n-1}) * P
        expected_cumul += (total_cells - expected_cumul) * P_activate;
        fcount << eqEta.step << "  " << eqEta.time << "  " << expected_cumul << "\n";
        if (writer.shouldPrint(eqEta.step)) fcount.flush();
    }
