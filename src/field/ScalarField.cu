...
void ScalarField::writeBinary(const std::string& path) const {
    std::ofstream ofs(path, std::ios::binary);
    if (!ofs)
        throw std::runtime_error("ScalarField::write: cannot open file: " + path);
...
void ScalarField::writeDat(const std::string& path) const {
...
void ScalarField::writeVts(const std::string& path) const {
    std::ofstream ofs(path);
    if (!ofs)
        throw std::runtime_error("ScalarField::write: cannot open file: " + path);
...
ScalarField ScalarField::readFromFile(const Mesh& mesh,
    ...
    if (!ifs)
        throw std::runtime_error("ScalarField::readFromFile: cannot open file: " + path);
    ...
    if (!headerDone)
        throw std::runtime_error("ScalarField::readFromFile: missing '---' header terminator in " + path);

    // Validate dimensions
    if (fnx != mesh.n[0] || fny != mesh.n[1] || fnz != mesh.n[2]) {
        throw std::runtime_error(
            "ScalarField::readFromFile: mesh dimensions in file ("
    ...
    ScalarField f(mesh, fieldName, ghost);
    ...
    if (!ifs)
        throw std::runtime_error("ScalarField::readFromFile: unexpected end of binary data in " + path);
